...
extern const char BrakeJenkins_LibIdent[];
const char BrakeJenkins_LibIdent[] = "(@@)" QUOTE(MODEL) " " ARCH " 1.0 " BUILDDATE;
...
static void
DoOneStep (rtModel_BrakeJenkins *rtm)
{
...
static void
BrakeJenkins_DeclQuants (void *MP)
{
    rtModel_BrakeJenkins *rtm = (rtModel_BrakeJenkins *)MP;
    int i;
	...
	/* Remember body frames defined in this model for later registration. */
        MdlBdyFrame_Add(BrakeJenkins_BdyFrameDefines);

	/* Define dict entries for non-dynamically allocated variables. */
	if ((i = MatSupp_DeclQuants(BrakeJenkins_DictDefines)) >= 0) {
	    LogErrF(EC_Init, "Model '%s': Error defining quantity '%s'",
		    Modelname, BrakeJenkins_DictDefines[i].Name);
	}
...
/*
 * BrakeJenkins_SetParams() will be invoked indirectly by the generated
 * model C code each time BrakeJenkins_New() is called.
 */
void
BrakeJenkins_SetParams (rtModel_BrakeJenkins *rtm, struct tMatSuppTunables *tuns,
		   struct tInfos *Inf)
{
    /*Log("%s_SetParams()\n", Modelname);*/
...
{
    rtModel_BrakeJenkins *rtm = (rtModel_BrakeJenkins *)MP;
    ExternalInputs_BrakeJenkins  *in  = (ExternalInputs_BrakeJenkins *) rtmGetU(rtm);
...
static void *
BrakeJenkins_New (struct tInfos *Inf, tBrakeCfgIF *CfgIF, const char *KindKey)
{
    rtModel_BrakeJenkins *rtm;
    ...
    MatSupp_ResetQuants(BrakeJenkins_DictDefines);
...
static int
BrakeJenkins_Calc (void *MP, tBrakeIF *IF, double dt)
{
    rtModel_BrakeJenkins *rtm = (rtModel_BrakeJenkins *)MP;
    int osCount=0;
    ExternalInputs_BrakeJenkins  *in  = (ExternalInputs_BrakeJenkins *) rtmGetU(rtm);
    ExternalOutputs_BrakeJenkins *out = (ExternalOutputs_BrakeJenkins *)rtmGetY(rtm);
...
BrakeJenkins_Register (void)
{
    ...
    m.Brake.DeclQuants		= BrakeJenkins_DeclQuants;
    m.Brake.New			= BrakeJenkins_New;
    m.Brake.Calc		= BrakeJenkins_Calc;
    m.Brake.Delete		= BrakeJenkins_Delete;
